...
| `browser_extension/app/src/background/download-suffix-filter.ts` | 浏览器原生下载后缀排除规则解析与匹配 |
| `browser_extension/app/src/background/popup-settings.ts` | popup 设置保存的轻量确认响应，避免等待完整状态刷新 |
| `browser_extension/app/src/shared/runtime-messages.ts` | popup runtime message 响应校验与错误归一化 |
| `browser_extension/app/tests/download-suffix-filter.test.ts` | 浏览器下载后缀过滤单元测试 |
| `AGENTS.md` | AI 代理完整协作说明与 Fork 同步保护清单 |
| `CLAUDE.md` | Claude Code / AI 代理快速读取的仓库级说明 |
...
| `.gitignore` | 新增 Rust 构建产物、AI 本地目录等忽略规则；保留仓库级 `AGENTS.md` / `CLAUDE.md` 可版本管理 |
| `.github/workflows/build.yml` | 新增 `build-browser-extension` job：测试、类型检查、构建、打包并上传 Chromium/Firefox 扩展资产 |
| `browser_extension/app/src/background.ts` | 浏览器下载接管前应用后缀排除规则；popup 设置保存使用轻量确认响应 |
| `browser_extension/app/src/popup/hooks/usePopupBridge.ts` | 保存设置后处理确认 payload，并容忍已知 runtime message 关闭错误 |
| `browser_extension/app/src/popup/components/SettingsPage.tsx` | 新增浏览器下载后缀排除设置 UI |
| `browser_extension/app/src/shared/types.ts` | `PopupStatePayload` 新增 `browserDownloadExcludedExtensions` 字段 |
...
- `deploy.py` 中 `format_pe_version(VERSION)` 用于 Nuitka/Windows PE 版本号字段：输出最多 4 段数字；Fork 迭代号作为第 4 段，上游版本已有 4 段时用迭代号替换第 4 段（如 `3.10.2.1-1` → `3.10.2.1`，`3.10-1` → `3.10.0.1`）

### 浏览器扩展发布资产

- Release 必须包含浏览器扩展资产：`Ghost-Downloader-v{VERSION}-BrowserExtension-Chromium.zip` 和 `Ghost-Downloader-v{VERSION}-BrowserExtension-Firefox.zip`
- `.github/workflows/build.yml` 中的 `build-browser-extension` job 是本 Fork 的发布关键路径，负责执行 `npm run test`、`npm run typecheck`、`npm run build`，并打包/上传上述两个 zip，**不得在同步上游时被覆盖或删除**
- 手动触发工作流时，`build_target` 必须保留 `browser_extension` 和 `all` 选项，便于单独重建扩展资产
- 发布前确认扩展资产与 `app/supports/config.py` 的 `VERSION` 一致，例如 `v3.10.2.1-3` 对应 `Ghost-Downloader-v3.10.2.1-3-BrowserExtension-*.zip`
...
## 浏览器扩展增强 (`browser_extension/app/`)

本 Fork 在浏览器扩展侧增强了浏览器原生下载接管与 popup 设置链路。同步上游时必须保留以下行为：

- `interceptDownloads` 仍是全局总开关；开启后才检查浏览器下载后缀排除规则
- `download-suffix-filter.ts` 从下载文件名、`finalUrl` 或 `url` 推断后缀，规则大小写不敏感，支持用户输入带点或不带点的后缀
- 命中 `browserDownloadExcludedExtensions` 的浏览器原生下载必须继续由浏览器处理，不应取消并交给桌面端；无法识别后缀时保持默认接管
- `popup_set_browser_download_suffix_filter` 这类简单设置保存命令应返回轻量确认 payload（如 `{ ok: true, message, browserDownloadExcludedExtensions }`），不要等待完整 `buildPopupState()`；完整状态由后续刷新同步，避免 popup 关闭或后台刷新较慢时出现 `message port closed before a response`
- popup 侧 runtime message 处理需继续使用 `runtime-messages.ts` 的响应校验和错误归一化，避免把 Chrome/Firefox 的英文通道关闭错误直接暴露给用户
...
# 浏览器扩展：测试、类型检查、构建 Chromium/Firefox 产物
cd browser_extension/app && npm run test && npm run typecheck && npm run build
```
...
This project is indexed by GitNexus as **Ghost-Downloader-3** (6598 symbols, 14087 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
